...
* `nodeBuiltins` _[object]_ - Configuration settings for polyfilling NodeJS built-ins(only `global`, `__filename` or `__dirname`). See `node` [webpack option](https://webpack.js.org/configuration/node/).
* `fallbackNodeBuiltins` _[object]_ - Configuration settings for polyfilling NodeJS built-ins. See `resolve.fallback` [webpack option](https://webpack.js.org/configuration/resolve/#resolvefallback).
