...
These tasks will execute all valid tests (files that end in `-specs.js`) that are within the project directory. The `test` is a standard execution pass and `test-watch` will set up a watcher to re-execute tests when files change.
